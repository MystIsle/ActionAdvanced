...
- **Melee hit detection** (`MeleeTrace` plugin) — `UMeleeTraceComponent` on `AAACharacter` + `UAnimNotifyState_MeleeTrace` on attack montages (swept sphere between socket positions across frames, per-trace dedup). The plugin only ignores the owner — **team filtering is the consumer's job**: `AAACharacter::OnMeleeTraceHit` (bound in `BeginPlay`) is the hit seam — it filters to Hostile and debug-draws. **Damage / hitstun / knockback ("경직/넉백") are an open TODO to wire there.** Trace channel (Pawn vs PhysicsBody vs a custom Melee channel) is still being decided.
